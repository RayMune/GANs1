...
def load_data(data_path):
  ...
  for filename in os.listdir(data_path):
    ...
    img = np.array(img.resize(
        input_shape[:2])) / 255.0  # Normalize the images to the range [0, 1]
    images.append(img)
...
def build_generator():
  ...
  model.add(
      layers.Dense(128 * 16 * 16, use_bias=False, input_shape=(latent_dim, )))
...
dataset = tf.data.Dataset.from_tensor_slices(dataset).shuffle(10000).batch(
    batch_size)
...
for epoch in range(epochs):
  ...
  if (epoch + 1) % 100 == 0:
    ...
    generated_image = tf.cast((generated_image[0] * 255), tf.uint8)
    Image.fromarray(
        generated_image.numpy()).save(f'generated_image_epoch_{epoch + 1}.png')
